    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-16">
        ...
        <div className="relative h-48 sm:h-64 overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-6 container">
            <Link to="/" className="flex items-center gap-2 text-primary-foreground/70 hover:text-primary-foreground text-sm mb-3 transition-colors w-fit">
              <ArrowLeft className="h-4 w-4" /> Início
            </Link>
            <h1 className="font-display text-3xl sm:text-4xl font-black text-primary-foreground">
              {category?.name || "..."}
            </h1>
            <p className="text-primary-foreground/70 mt-1 max-w-md">{category?.description}</p>
            {minPrice !== null && (
          ...
          {isLoading ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {flavors?.map((flavor) => (
                ...
                >
                  <div className="h-44 bg-gradient-to-br from-primary/10 to-accent flex items-center justify-center overflow-hidden">
                    {flavor.image_url ? (
