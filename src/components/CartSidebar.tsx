...
export default function CartSidebar() {
  ...
  const isBelowMin = totalPrice > 0 && totalPrice < storeConfig.minOrderValue;
                ...
                <div key={key} className="flex gap-3 bg-muted rounded-xl p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-0.5">
                      {item.selectedSize && (
                        <span className="text-[10px] bg-primary/10 text-primary px-1.5 py-0.5 rounded">{item.selectedSize.label}</span>
                      )}
                      {item.selectedFlavor && (
                        <span className="text-[10px] bg-secondary/10 text-secondary-foreground px-1.5 py-0.5 rounded">{item.selectedFlavor.label}</span>
                      )}
                    </div>
          ...
          <div className="p-4 border-t border-border space-y-3">
            {isBelowMin && (
              <div className="bg-secondary/10 text-secondary-foreground text-sm px-3 py-2 rounded-lg text-center">
                ⚠️ Pedido mínimo: R$ {storeConfig.minOrderValue.toFixed(2).replace(".", ",")}
              </div>
            )}
